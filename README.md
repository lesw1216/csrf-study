...
2. attack.html 독립적으로 실행되어야함.
    ...
    ```
   python -m htpp.server 4000 // 40000 포트로 열기
   ```
   
    attack.html 파일 위치로 이동하여 터미널로 위 명령어 실행, http://127.0.0.1:40000/attack.html 접속
   (서로 도메인이 달라야 하기 때문에 127.0.0.1:40000 사용)

위 조건에서 테스트를 진행하면 된다. ngrok을 사용하여 스프링 부트를 https로 호출해보는 것도 나쁘지 않는 방법


# 결과

attack.html 세 가지 공격 방법을 우선 나열한다.
